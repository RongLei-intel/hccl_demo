 ...
 ******************************************************************************/
 ...
 #include <chrono> // Add this header for getTimestamp implementation
 ...
 static hcclResult_t sendRecvRanksTest(uint64_t                     iter,
                                       ...
                                       std::vector<uint64_t>&       iterTimestamps) // Changed from double to uint64_t
 {
     // timestamp 1
     uint64_t timestamp = getTimestamp();
     iterTimestamps.push_back(timestamp); 
 
     hcclGroupStart();
 
     // timestamp 2
     uint64_t groupStartTimestamp = getTimestamp();
     iterTimestamps.push_back(groupStartTimestamp); 
     ...
     // timestamp 3
     uint64_t groupEndTimestamp = getTimestamp();
     iterTimestamps.push_back(groupEndTimestamp); 
 
     hcclGroupEnd();
     
     //timestamp 4
     uint64_t groupAfterEndTimestamp = getTimestamp();
     iterTimestamps.push_back(groupAfterEndTimestamp); 
                              ...
                              Stats&                 stats)
 {
     ...
     std::vector<std::vector<uint64_t>> groupDurations(envData.numIters, std::vector<uint64_t>()); // Initialize as empty vectors
     ...
     stats.rankDurationInSec = benchmark(
         ...
         [&](uint64_t iter) {
             uint64_t index = iter % buffers.inputDevPtrs.size();
             // Ensure the lambda returns hcclResult_t
     ...
     if (recvFromRanks.size() > 0 || sendToRanks.size() > 0) 
     {
        
         // Fix the average time calculations
         uint64_t averageGroupStartTime = 0;
         uint64_t averageGroupTime = 0;
         uint64_t averageGroupEndTime = 0;
         uint64_t averageGroupTotalTime = 0;
 
         for (size_t i = 0; i < groupDurations.size(); i++)
         {
             if (groupDurations[i].size() > 0)
             {
                 averageGroupStartTime += groupDurations[i][1] - groupDurations[i][0];
                 averageGroupTime += groupDurations[i][2] - groupDurations[i][1];
                 averageGroupEndTime += groupDurations[i][3] - groupDurations[i][2];
                 averageGroupTotalTime += groupDurations[i][3] - groupDurations[i][0];
             }
         }
 
         if (groupDurations.size() > 0)
         {
             averageGroupStartTime /= groupDurations.size();
             averageGroupTime /= groupDurations.size();  
             averageGroupEndTime /= groupDurations.size();
             averageGroupTotalTime /= groupDurations.size();
         }
         std::string sendToRanksString;
         for(size_t i = 0; i < sendToRanks.size(); i++)
         {
             sendToRanksString += std::to_string(sendToRanks[i]);
             if (i < sendToRanks.size() - 1)
             {
                 sendToRanksString += ",";
             }
         }
         std::string recvFromRanksString;
         for(size_t i = 0; i < recvFromRanks.size(); i++)
         {
             recvFromRanksString += std::to_string(recvFromRanks[i]);
             if (i < recvFromRanks.size() - 1)
             {
                 recvFromRanksString += ",";
             }
         }
         // print detail info
         log() << "SendRecvRanksTest: "
             << " SendToRanks: " << sendToRanksString
             << ", RecvFromRanks: " << recvFromRanksString
             << ", current_rank: " << envData.rank
             << ", iterations: " << groupDurations.size()
             << ", Size: " << size 
             << ", Average group start time: " << averageGroupStartTime / 1000.0 << " ms"
             << ", Average group time: " << averageGroupTime / 1000.0 << " ms"
             << ", Average group end time: " << averageGroupEndTime / 1000.0 << " ms"
             << ", Average group total time: " << averageGroupTotalTime / 1000.0 << " ms"
             << std::endl;
     }
 }
